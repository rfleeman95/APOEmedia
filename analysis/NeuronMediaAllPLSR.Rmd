...
#If using genotype:
class <- My_csv$Concentration[1:93]
...
ggplot(My_csv_PC,aes(x=PC1,y=PC2,col=class))+
  ...
  #theme(legend.key.size = unit(0,'lines'))+ #this makes the key all weird on a continuous scale so only use for PLSDA
  #theme(legend.justification = c(0,1), legend.position = c(0.01,1))+ #this moves the key from the right side to the top which didn't fit well
  geom_hline(yintercept = 0, linetype="dashed", color="black",size=0.5)+
  geom_vline(xintercept = 0,linetype="dashed",color="black",size=0.5)+
  xlim(-10,17)+
  ylim(-7,5)+
  scale_colour_gradient(low = "darkorange", high = "blue") #this is for continuous scale
  #scale_colour_manual(values = c("blue","darkorange")
  #scale_colour_manual(values = c("blue","darkorange", "coral1", "deepskyblue", "deeppink3", "chartreuse1", "darkorchid4", "goldenrod1", "aquamarine4", "firebrick2"))
...
#length(Loadings_PC1)
Load_PC1=as.matrix(Loadings_PC1[1:11])
Load_PC1=t(Load_PC1)
barplot(Load_PC1,col="grey89",las=2,ylim=c(-0.1,0.4))
abline(h=0)

Loadings_PC2=PC$rotation[,2]
#Loadings_PC2=sort(Loadings_PC2,decreasing=TRUE)
#length(Loadings_PC2)
Load_PC2=as.matrix(Loadings_PC2[1:11])
Load_PC2=t(Load_PC2)
barplot(Load_PC2,col="grey89",las=2,ylim=c(-0.6,0.4))
abline(h=0)

Loadings_PC3=PC$rotation[,3]
#Loadings_PC3=sort(Loadings_PC3,decreasing=TRUE)
#length(Loadings_PC3)
Load_PC3=as.matrix(Loadings_PC3[1:11])
Load_PC3=t(Load_PC3)
...
oplsda<-opls(My_csv_quant,y=class,predI=1,orthoI=1,crossvalI=8)


#SCORES PLOT:
Scores <- getScoreMN(oplsda)
oScores <- oplsda@orthoScoreMN
Scores_Class <- data.frame(Scores, oScores, My_csv$Concentration)
...
ggplot(Scores_Class,aes(x=LV1,y=o1,col=Class))+
  ...
  theme(axis.line = element_line(colour = "black",size=0.5))+
  theme(legend.title=element_blank())+
  scale_colour_gradient(low = "blue", high = "darkorange")+ #this is for continuous scale
  ...
  xlim(-4,4)+
  ylim(-5,5)+
  xlab("Scores on LV 1 (19.9%)") +     #change this to match input LV!!!
  ylab("Scores on LV 2 (43.6%)")+     #change this to match input LV!!!
...
Loading<-t(Load)
barplot(Loading, col="grey89", las=2, ylim=c(-0.6,0.6), ylab="LV1 (Component) 19.1%", font.lab=2)
#!!CHANGE TITLE AND LV %
abline(h=0)

VIP<-getVipVn(oplsda)
VIPsort=sort(VIP,decreasing=TRUE)
barplot(VIPsort, col="grey89",las=2, ylim=c(0,2), main= "VIPs", cex.main=1.5, ylab="LV1 (Component) 19.1%", font.lab=2)
#!!CHANGE TITLE AND LV %
abline(h=1)
abline(h=0)
